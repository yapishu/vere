...
{
  u3_disk*     log_u = mar_u->log_u;
  u3_mars_boot_opts inp_u;
  u3_mars_boot_meta bom_u;
  u3_meta           met_u;
  u3_noun   com, ova, cax;
  ...
  if ( c3n == u3_mars_boot_make(&inp_u, com, &ova, &cax, &bom_u) ) {
    fprintf(stderr, "boot: preparation failed\r\n");
    exit(1);  //  XX cleanup
  }

  met_u.ver_w = bom_u.ver_w;
  met_u.who_d[0] = bom_u.who_d[0];
  met_u.who_d[1] = bom_u.who_d[1];
  met_u.fak_o = bom_u.fak_o;
  met_u.lif_w = bom_u.lif_w;
