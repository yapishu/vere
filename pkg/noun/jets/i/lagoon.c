...
*/
                   ...
                   )
  {
    ...
    // Hoon `(sub a b)` = a - b. AXPY computes Y := alpha*X + Y.
    // To get Y = a - b we set X = b (alpha = -1), Y = a; then Y becomes a - b.
    // a (x_data) is loaded into a-with-pin (the accumulator), b (y_data) into
    // a no-pin scratch buffer.
