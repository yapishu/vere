...
*/
                   ...
                   )
  {
    ...
    // Hoon `(sub a b)` = a - b. AXPY computes Y := alpha*X + Y; with
    // alpha=-1 the accumulator Y becomes Y - X. To get a - b we therefore
    // load `a` (x_data) into the Y accumulator (with the MSB pin byte) and
    // `b` (y_data) into the X buffer (no pin needed; only read).
    // The previous ordering had `a` as X and `b` as Y, which silently
    // computed `b - a` — see sub-scalar below for the correct pattern.
    c3_y* b_bytes = (c3_y*)u3a_malloc(syz_x*sizeof(c3_y));
    u3r_bytes(0, syz_x, b_bytes, y_data);

    c3_y* a_bytes = (c3_y*)u3a_malloc((syz_x+1)*sizeof(c3_y));
    u3r_bytes(0, syz_x+1, a_bytes, x_data);

    //  Switch on the block size.
    switch (u3x_atom(bloq)) {
      case 4:
        haxpy(len_x, (float16_t){SB_REAL16_NEGONE}, (float16_t*)b_bytes, 1, (float16_t*)a_bytes, 1);
        break;

      case 5:
        saxpy(len_x, (float32_t){SB_REAL32_NEGONE}, (float32_t*)b_bytes, 1, (float32_t*)a_bytes, 1);
        break;

      case 6:
        daxpy(len_x, (float64_t){SB_REAL64_NEGONE}, (float64_t*)b_bytes, 1, (float64_t*)a_bytes, 1);
        break;

      case 7:
        qaxpy(len_x, (float128_t){SB_REAL128L_NEGONE,SB_REAL128U_NEGONE}, (float128_t*)b_bytes, 1, (float128_t*)a_bytes, 1);
        break;
    }

    // r_data is the result noun of [data]
    u3_noun r_data = u3i_bytes((syz_x+1)*sizeof(c3_y), a_bytes);
